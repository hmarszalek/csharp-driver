...
pub(crate) struct BridgedSessionInner {
    // Optional connected session state to allow for shutdown.
...
pub extern "C" fn session_create(tcb: Tcb, uri: CSharpStr<'_>) {
    ...
    BridgedFuture::spawn::<_, _, NewSessionError>(tcb, async move {
        ...
        Ok(RwLock::new(BridgedSessionInner {
            connected: Some(ConnectedSession { session }),
        }))
    })
}
...
pub extern "C" fn session_shutdown(
    ...
    BridgedFuture::spawn::<_, _, SessionShutdownError>(tcb, async move {
        ...
        // Acquire write lock - this will pause the asynchronous execution until all read locks
        // (queries) are released and then clear the connected state - no more queries can
        // proceed after this.
...
pub extern "C" fn session_prepare(
    ...
    BridgedFuture::spawn::<_, _, MaybeShutdownError<PrepareError>>(tcb, async move {
        ...
        let Some(connected) = session_guard.connected.as_ref() else {
            return Err(MaybeShutdownError::AlreadyShutdown);
        };
